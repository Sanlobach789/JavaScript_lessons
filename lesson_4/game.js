...
const player = {
    ...
    alertMessage(){
        alert('Нельзя двигаться за пределы карты')
    },
    checkPosition(direction){
        if (direction===8 && this.y===0){
            this.alertMessage();
        }
        else if (direction===4 && this.x===0){
            this.alertMessage();
        }
        else if (direction===6 && this.x===9){
            this.alertMessage();
        }
        else if (direction===2 && this.y===9){
            this.alertMessage();
        }
        else {
            this.move(direction)
...
const game = {
    ...
    run() {
        ...
        while (true) {
            ...
            this.player.checkPosition(direction);
        }
    },
